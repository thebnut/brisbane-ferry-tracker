...
const MODE_CONFIGS = {
  ...
  train: {
    ...
    outputDir: 'schedule-data-transit/train',
  },
  bus: {
    routeType: 3,
    routePrefix: null, // Buses use numeric route IDs
    outputDir: 'schedule-data-transit/bus',
  }
};
...
// Extract platform information from train stop names
function extractPlatformInfo(stopName) {
  // Pattern: "Central station, platform 3" -> { station: "Central station", platform: "3" }
  const platformMatch = stopName.match(/^(.+),\s*platform\s+(\d+)$/i);
  if (platformMatch) {
    return {
      stationName: platformMatch[1].trim(),
      platform: platformMatch[2]
    };
  }
  // No platform info found
  return {
...
// Group train platforms into logical stations
function groupPlatformsIntoStations(modeStops) {
  if (mode !== 'train') {
    return modeStops; // No grouping for non-train modes
  }

  console.log('Grouping train platforms into logical stations...');
  const stations = {};
  const platformToStation = {}; // Map platform ID to station ID

  // Group platforms by station name
  Object.entries(modeStops).forEach(([platformId, platformData]) => {
    const { stationName, platform } = extractPlatformInfo(platformData.name);

    // Create station ID from name (lowercase, replace spaces with hyphens)
    ...
    stations[stationId].platforms.push({
      id: platformId,
      platform: platform,
      name: platformData.name
    });

    // Map platform to station for connectivity
    platformToStation[platformId] = stationId;

    // Merge valid destinations (convert to Set for deduplication)
    if (platformData.validDestinations) {
      platformData.validDestinations.forEach(destId => {
        stations[stationId].validDestinations.add(destId);
      });
    }
  });

  // Convert station destinations from platform IDs to station IDs
  Object.values(stations).forEach(station => {
    const stationDestinations = new Set();
    station.validDestinations.forEach(platformId => {
      const destStationId = platformToStation[platformId];
      if (destStationId && destStationId !== station.id) {
        stationDestinations.add(destStationId);
      }
    });
    station.validDestinations = Array.from(stationDestinations).sort();
  });

  console.log(`Grouped ${Object.keys(modeStops).length} platforms into ${Object.keys(stations).length} stations`);

  return { stations, platformToStation };
}
...
async function processGTFSData() {
  ...
  // For train mode, group platforms into stations
  let stationsData = null;
  let platformToStation = null;
  if (mode === 'train') {
    const stationGrouping = groupPlatformsIntoStations(modeStops);
    stationsData = stationGrouping.stations;
    platformToStation = stationGrouping.platformToStation;

    // Add platform metadata to departures
    departures.forEach(departure => {
      const platformData = modeStops[departure.stopId];
      if (platformData) {
        const { stationName, platform } = extractPlatformInfo(platformData.name);
        departure.platformNumber = platform;
        departure.stationId = platformToStation[departure.stopId];
  ...
  const output = {
    ...
    stops: mode === 'train' ? stationsData : modeStops, // Use stations for train mode, stops for others
    stopConnectivity: stopConnectivity
  };

  // Add platform mapping for train mode
  if (mode === 'train' && platformToStation) {
    output.platformToStation = platformToStation;
    output.platforms = modeStops; // Keep platform data for reference
  }
